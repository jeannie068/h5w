...
struct Cluster {
    ...
    double q_value;         // q_c
    double optimal_x;       // calculated optimal x position
    
    Cluster() : first_cell_index(-1), last_cell_index(-1), 
                total_weight(0.0), total_width(0.0), q_value(0.0), optimal_x(0.0) {}
};
...
struct SubRow {
    ...
    SubRow(int parent_idx, double sx, double ex, int row_y, int row_h, int sw)
        : parent_row_index(parent_idx), start_x(sx), end_x(ex), 
          y(row_y), height(row_h), site_width(sw) {
